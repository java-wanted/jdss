...
class Kmax(Solver):
    ...
    def get_mcount(self, sm: np.ndarray, oi=-1) -> np.ndarray:
        """Caclulate times k-maximum variants are optimal.

        It is not stated what the maximum and largest optimality of variants mean.
        Although, it has meaning for (j,k) only.
        """

        def pmax(j: int) -> tuple[bool, bool]:
            er = np.min(sm, axis=1)
            jm = np.argmax(er)
            vm = er[jm]
            er[jm] = 0
            return bool(np.all(er < vm)), j == jm

        def pgre(j: int) -> tuple[bool, bool]:
            er = np.max(sm, axis=1)
            jm = np.argmax(er)
            vm = er[jm]
            er[jm] = 0
        ...
        mj = np.argmax(sm, axis=0)
        for j in mj:
            o = np.array([*pmax(j), *pgre(j)], dtype=int)
            mc[j] = np.sum(o) if oi == -1 else o[oi]
        return mc
    ...
    def solve(self, xp: np.ndarray, inc: np.ndarray, wgt: np.ndarray) -> None:
        ...
        for pi in range(p):
            ...
            mc = self.get_mcount(sm, op)
            sjm = np.zeros(n, dtype=float)
            for j in range(n):
                sjm[j] = sj[j] * mc[j]
